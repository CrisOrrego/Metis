...
.controller('CubosCtrl', ['$scope', '$rootScope', '$injector', '$mdDialog', 
	function($scope, $rootScope, $injector, $mdDialog) {
		...
		//Validacion permiso
		if(!Rs.checkPerm('level_cubos', 'see')) Rs.Logout();
		...
		//Agregar limitantes de cubos
		const access_cubos = Rs.Usuario.perfil.Config.access_cubos;
		if(access_cubos == 'A'){
			Ctrl.CubosCRUD.ops.query_scopes.push([ 'onlyids', Rs.Usuario.perfil.Config.access_cubos_list ]);
		}else if(access_cubos == 'N'){
			Ctrl.CubosCRUD.ops.query_scopes.push([ 'onlyids', [] ]);
		};
...
.controller('InformesCtrl', ['$scope', '$rootScope', '$http', '$injector', '$timeout', '$location', 'clipboard', '$mdDialog',
	function($scope, $rootScope, $http, $injector, $timeout, $location, clipboard, $mdDialog) {
		...
		//Validacion permiso
		if(!Rs.checkPerm('level_informes', 'see')) Rs.Logout();
		...
		Ctrl.BASE_URL = Rs.Opts.APP_ROUTE + '/#/I/';
		Ctrl.copyUrl = function(){
			var Url =  Ctrl.BASE_URL + Ctrl.Informe.Url;
...
angular.module('Panel_Details_ChangeTipoCtrl', [])
.controller('Panel_Details_ChangeTipoCtrl', ['$scope', '$rootScope', '$mdDialog', 'Config', 
	function($scope, $rootScope, $mdDialog, Config) {
		...
		Ctrl.Cancel = function(){ $mdDialog.cancel(); }
		Ctrl.Res = function(d){ $mdDialog.hide(d); }

		Ctrl.TiposPanel = Rs.TiposPanel;
	}
]);
...
.controller('Panel_PanelDataTableCtrl', ['$scope', '$rootScope', 
	function($scope, $rootScope) {
		...
		Ctrl.orderer = { id: '', dir: '', value: '' };
		Ctrl.order = (v) => {
			if(v == Ctrl.orderer.id){ //Igual
				Ctrl.orderer.dir = (Ctrl.orderer.dir == '') ? '-' : '';
			}else{
				Ctrl.orderer.dir = '-';
			};

			Ctrl.orderer.id = v;
			Ctrl.orderer.value = Ctrl.orderer.dir + v;
		};
	}
]);
...
.controller('PanelesCtrl', ['$scope', '$rootScope', '$http', '$mdDialog', '$injector', '$timeout',
	function($scope, $rootScope, $http, $mdDialog, $injector, $timeout) {
		...
		//Validacion permiso
		if(!Rs.checkPerm('level_paneles', 'see')) Rs.Logout();
		...
		Ctrl.getCubos = () => {
			Ctrl.CubosCRUD.get().then(function(){
				if(Ctrl.CubosCRUD.rows.length > 0){
					Ctrl.getPanels(Ctrl.CubosCRUD.rows[0]);
				};
			});
		};

		//Limitar los cubos accesibles
		const access_cubos = Rs.Usuario.perfil.Config.access_cubos;
		if(access_cubos == 'A'){
			Ctrl.CubosCRUD.ops.query_scopes.push([ 'onlyids', Rs.Usuario.perfil.Config.access_cubos_list ]);
			Ctrl.getCubos();
		}else if(access_cubos == 'T'){
			Ctrl.getCubos();
		};
		...
		Ctrl.addPanel = function(config){
			...
			Ctrl.PanelesCRUD.dialog({
				...
				//Obtener el tipo
				Ctrl.openTipoDiag({ Titulo: 'Tipo de Panel', ev: config.ev }).then((t) => {
					newElm.Tipo = t;
					newElm.Config = angular.copy(Ctrl.PanelesConfig[t]._default);
		...
		Ctrl.openTipoDiag = function(Config){
			return $mdDialog.show({
				controller: 'Panel_Details_ChangeTipoCtrl',
		...
		Ctrl.Cancel = function(){ $mdDialog.cancel(); }
		Ctrl.changeTipoPanel = function(ev){

			Ctrl.openTipoDiag({ Titulo: 'Cambiar Tipo de Panel', ev: ev }).then((t) => {
				if(t !== Ctrl.Panel.Tipo){
					Ctrl.Panel.Tipo = t;
					Ctrl.Panel.Config = angular.copy(Ctrl.PanelesConfig[t]._default);
					Ctrl.Panel.Data = null;
					Ctrl.showPanelPreview = false;
				};
			});
...
.controller('InicioCtrl', ['$scope', '$rootScope', '$http', '$mdDialog',
	function($scope, $rootScope, $http, $mdDialog) {

		console.info('InicioCtrl');
		var Ctrl = $scope;
		var Rs = $rootScope;
		Ctrl.showInicioNav = true;

		Ctrl.BASE_URL = Rs.Opts.APP_ROUTE + '/#/I/';

		Rs.http('/api/Informe/all', { validatePerm: true }, Ctrl, 'Informes').then(() => {
			//if(Ctrl.Informes.length > 0) Ctrl.openInforme(Ctrl.Informes[0]);
		});
		...
		Ctrl.toogleNavbar = function(){
			Ctrl.showInicioNav = !Ctrl.showInicioNav;
		};
